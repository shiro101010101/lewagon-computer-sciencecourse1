...
def parse_recipe(article):#returnで何か返すためにこの値を入れてる
...
def scrape_from_file(ingredient):
    ...
    if path.exists(file):
        return open(file)#return to html file
    print("Please, run the following command first:")
    print(
        f'curl "https://recipes.lewagon.com/?search[query]={ingredient}" > pages/{ingredient}.html'
    )
    sys.exit(1)
    #error
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        recipes = parse(scrape_from_file(ingredient)) #parse(returned html.file)
